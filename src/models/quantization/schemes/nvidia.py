...
class Nvidia_fn(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        # EW
        e_fp4 = sr_1x16s_fp4_kernel_wrapper(grad_output, Nvidia_fn.hadamard_matrix.to(grad_output.dtype), (17 / 16), Nvidia_fn.group_size)
        ...
        e_tht_fp4 = sr_1x16s_fp4_kernel_wrapper(e_tht, Nvidia_fn.hadamard_matrix.to(grad_output.dtype), (17 / 16), Nvidia_fn.group_size)
        
        input_tht = (input_fp4.T.reshape(-1, Nvidia_fn.hadamard_matrix.size(0)) @ Nvidia_fn.hadamard_matrix.T).reshape(ctx.in_dim, ctx.batch * ctx.seq)
        input_tht_fp4 = rtn_1x16s_fp4_kernel_wrapper(input_tht, Nvidia_fn.hadamard_matrix.to(input_fp4.dtype), 1.0, Nvidia_fn.group_size)
