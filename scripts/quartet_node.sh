...
#SBATCH --no-requeue
#SBATCH --output=/capstor/store/cscs/swissai/a140/codebases/backprop-scaling-laws/logs/slurm/quartet_sweep/sweep-%A_%a.out
#SBATCH --error=/capstor/store/cscs/swissai/a140/codebases/backprop-scaling-laws/logs/slurm/quartet_sweep/sweep-%A_%a.err
...
# Parse Quant Setup
IFS=":" read -r STOCHASTIC <<< "$CURRENT_SETUP"
...
export G_BITS=4
export G_QUANT_KWARGS="{\"hadamard_dim\": 32, \"rerotate\": \"signs\", \"stochastic\": ${STOCHASTIC}}"
...
# WandB Prefix
SETUP_STR="${STOCHASTIC}"
WANDB_PREFIX="${MODEL_SIZE_PREFIX}-TOK${TOKENS}-${G_QUANT}-signfix@${G_BITS}@${SETUP_STR}-${DATASET}"
