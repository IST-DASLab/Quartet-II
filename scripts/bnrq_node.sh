...
#SBATCH --no-requeue
#SBATCH --output=/capstor/store/cscs/swissai/a140/codebases/backprop-scaling-laws/logs/slurm/bnrq_sweep/bw-sweep-%A_%a.out
#SBATCH --error=/capstor/store/cscs/swissai/a140/codebases/backprop-scaling-laws/logs/slurm/bnrq_sweep/bw-sweep-%A_%a.err
...
# Parse Quant Setup
IFS=":" read -r HADAMARD_DIM GROUP_DIM SCALE_DTYPE UNBIASED SCALE_OVERRIDE <<< "$CURRENT_SETUP"
...
echo "Quant: Hadamard Dim=${HADAMARD_DIM}, Group Dim=${GROUP_DIM}, Scale=${SCALE_DTYPE}, Unbiased=${UNBIASED}, Scale Override=${SCALE_OVERRIDE}"
...
export BACKWARD_SCHEME="Q(E)W_EtX"
...
# WandB Prefix
SETUP_STR="${HADAMARD_DIM};${GROUP_DIM};${SCALE_DTYPE};${UNBIASED};${SCALE_OVERRIDE}"
WANDB_PREFIX="${MODEL_SIZE_PREFIX}-TOK${TOKENS}-${G_QUANT}-triton-rerun@${G_BITS}@${SETUP_STR}-Q(E)W_EtX-${DATASET}"
